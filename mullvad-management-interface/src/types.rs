...
impl TryFrom<Relay> for mullvad_types::relay_list::Relay {
    ...
    fn try_from(relay: Relay) -> Result<Self, Self::Error> {
        use mullvad_types::relay_list::RelayEndpointData as MullvadEndpointData;
        use mullvad_types::relay_list::Relay as MullvadRelay;
        use mullvad_types::location::Location as MullvadLocation;

        let endpoint_data = match relay.endpoint_type {
            i if i == relay::RelayType::Openvpn as i32 => {
                MullvadEndpointData::Openvpn
            }
            i if i == relay::RelayType::Bridge as i32 => {
                MullvadEndpointData::Bridge
            }
            i if i == relay::RelayType::Wireguard as i32 => {
                let data = relay.endpoint_data.ok_or(FromProtobufTypeError::InvalidArgument(
                ...
                MullvadEndpointData::Wireguard(mullvad_types::relay_list::WireguardRelayEndpointData {
        ...
        let ipv6_addr_in = if relay.ipv6_addr_in.is_empty() {
            ...
            Some(relay.ipv4_addr_in.parse().map_err(|_err| FromProtobufTypeError::InvalidArgument(
        ...
        Ok(MullvadRelay {
            hostname: relay.hostname,
            ipv4_addr_in: relay.ipv4_addr_in.parse().map_err(|_err| FromProtobufTypeError::InvalidArgument(
                "invalid relay IPv4 address",
            ))?,
            ipv6_addr_in,
            include_in_country: relay.include_in_country,
            active: relay.active,
...
fn to_proto_any<T: prost::Message>(
    type_name: &str,
    message: T,
) -> prost_types::Any {
    prost_types::Any {
